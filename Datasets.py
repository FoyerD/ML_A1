...
def test_model_X_y(X, y, transform_label_func):
    ...
    weighted_gs = GridSearchCV(WeightedSoftDecisionTreeClassifier(), mega_param_grid, cv=10)
    soft_gs = GridSearchCV(SoftDecisionTreeClassifier(), mega_param_grid, cv=10)

    weighted_gs.fit(X_train.values, y_train)
    weighted_model = WeightedSoftDecisionTreeClassifier(n_runs=weighted_gs.best_estimator_["n_runs"], alpha=weighted_gs.best_estimator_["alpha"])
    soft_gs.fit(X_train.values, y_train)
    soft_model = SoftDecisionTreeClassifier(n_runs=soft_gs.best_estimator_["n_runs"], alpha=soft_gs.best_estimator_["alpha"])
    ...
    print("parameters: " + str(soft_gs.best_params_) + "\n")
    print("Weighted descision tree classifier:")
    ...
    print("parameters: " + str(weighted_gs.best_params_) + "\n")
    ...
    if(len(classes) <= 2):
        plot_roc_curve(y_test, y_pred_soft, y_pred_hard, y_pred_weight)
    else:
        pass #Not working
        #plot_roc_multi_curve(y_test,y_pred_soft, y_pred_hard, reverse_transform_label_func, list(classes))

    confusion_matrix_plot(y_test, y_pred_soft, y_pred_hard, y_pred_weight)
...
def plot_roc_curve(y_test, y_pred_soft, y_pred_hard, y_pred_weighted):
...
def confusion_matrix_plot(y_test, y_pred_soft, y_pred_hard, y_pred_weight):
    cm_soft = confusion_matrix(y_test, y_pred_soft)
    cm_hard = confusion_matrix(y_test, y_pred_hard)
    cm_weight = confusion_matrix(y_test, y_pred_soft, y_pred_weight)
    ...
    axes[1].set_title("Weighted Decision Tree Classifier Confusion Matrix")
...
def test_model_X_y_reg(X, y):
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.2)
    soft_model = GridSearchCV(SoftDecisionTreeRegressor(), mega_param_grid, cv=10)
...
def test_model_wine(dataset):
    print(" ----- Dataset 6 - Water quality -----")
    test_model_UCI(dataset, transform_label_wine, encode=True)
    print("")
...
def regression_data():
    ...
    wine_quality = fetch_ucirepo(id=186)
    test_model_UCI_reg(wine_quality, encode=True)
    print("")
